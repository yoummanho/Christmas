...
public class KW_DepthRendering : MonoBehaviour
{
    ...
    public void RenderWaterMaskDepthNormal2(Camera currentCamera, float resolutionScale, Mesh mesh, Matrix4x4 worldMatrix, bool useTesselation, Dictionary<CommandBuffer, CameraEvent> waterSharedBuffers, bool forceUpdate)
    {
        ...
        Shader.SetGlobalTexture(KW_WaterDepth_ID, waterDepthRT);
        Shader.SetGlobalTexture(KW_WaterMaskScatterNormals_ID, waterMaskRT);

        Graphics.SetRenderTarget(waterMaskRT.colorBuffer, waterDepthRT.depthBuffer);
        GL.Clear(true, true, Color.black);

        var shaderPass = useTesselation && SystemInfo.graphicsShaderLevel >= 46 ? 0 : 1;
        maskDepthNormalMaterial.SetPass(shaderPass);
        maskDepthNormalMaterial.EnableKeyword("IGNORE_TESS_CULL");

        GL.PushMatrix();
        GL.LoadProjectionMatrix(currentCamera.projectionMatrix);

        Graphics.DrawMeshNow(mesh, worldMatrix);

        GL.PopMatrix();
        if (pyramidBlurMask == null) pyramidBlurMask = new KW_PyramidBlur();
        pyramidBlurMask.ComputeBlurPyramid(3.0f, waterMaskRT, waterMaskRT_Blured);
        Shader.SetGlobalTexture(KW_WaterMaskScatterNormals_Blured_ID, waterMaskRT_Blured);
