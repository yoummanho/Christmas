...
public class KW_Buoyancy : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        for (int i = 0; i < voxels.Count; i++)
        {
            ...
            var waterSurfaceData =  water.GetWaterSurfacePositionNormal(wp);
            Vector3 force = Vector3.zero;

            if (waterSurfaceData != null)
            {
                var waterPos = ((KW_WaterSurfaceData)waterSurfaceData).Position;
                ...
                var surfaceNormal = ((KW_WaterSurfaceData)waterSurfaceData).Normal;
