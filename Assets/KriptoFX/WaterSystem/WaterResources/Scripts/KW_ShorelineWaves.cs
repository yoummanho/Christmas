...
public class KW_ShorelineWaves : MonoBehaviour
{
    ...
    public async Task BakeWavesToTexture(int areaSize, Vector3 shorelineAreaPos, float curvedSurfacesQualityScale, Vector3 waterPos, string GUID)
    {
        ...
        for (var i = 0; i < shorelineWavesData.Count; i++)
        {
            ...
            var roundedPos = new Vector2(Mathf.Round(shorelineWavesData[i].PositionX / metersInPixels) * metersInPixels, Mathf.Round(shorelineWavesData[i].PositionZ / metersInPixels) * metersInPixels);
            var roundedScale = new Vector2(Mathf.Round(shorelineWavesData[i].ScaleX / metersInPixels) * metersInPixels, Mathf.Round(shorelineWavesData[i].ScaleZ / metersInPixels) * metersInPixels);

            var position = new Vector3(roundedPos.x, 0, roundedPos.y);
            var rotation = Quaternion.Euler(270, 0, shorelineWavesData[i].EulerRotation);
            ...
            var targetMesh = shorelineWavesData[i].ID % 2 == 0 ? shorelineDataMesh1 : shorelineDataMesh2;
            targetMesh.WriteDataQuad(position, rotation, scale, uvOffset, new Vector2(scaleMultiplier, shorelineWavesData[i].TimeOffset), new Vector2((rotation.eulerAngles.y % 360f) / 360f, 0));
        }
        ...
        Graphics.SetRenderTarget(new[] { waves1_rt.colorBuffer, wavesData1_rt.colorBuffer }, waves1_rt.depthBuffer);
        GL.Clear(false, true, new Color(0, 0, 0, 0));
        Graphics.DrawMeshNow(tempMesh1, Matrix4x4.identity);

        Graphics.SetRenderTarget(new[] { waves2_rt.colorBuffer, wavesData2_rt.colorBuffer }, waves2_rt.depthBuffer);
        GL.Clear(false, true, new Color(0, 0, 0, 0));
    ...
    void OnDisable ()
    {
		KW_Extensions.SafeDestroy(waveMaterial, vatMaterial);
    ...
    {
        KW_Extensions.SafeDestroy(waves_tex1, waves_tex2, wavesData1_tex, wavesData2_tex, depth_tex, depthCamGO);
    ...
    {
        waves1_rt = KW_Extensions.ReinitializeRenderTexture(waves1_rt, size, size, 0, RenderTextureFormat.ARGBHalf, null, false, false, TextureWrapMode.Clamp);
        waves2_rt = KW_Extensions.ReinitializeRenderTexture(waves2_rt, size, size, 0, RenderTextureFormat.ARGBHalf, null, false, false, TextureWrapMode.Clamp);
