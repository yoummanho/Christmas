...
public class KW_WaterVolumetricLighting : MonoBehaviour
{
    ...
    private GameObject shadowsForceUpdateAnchor;
    ...
    public void Release()
    {
        ...
        KW_Extensions.ReleaseRenderTextures(VolumeLightTextureRaw, volumeLightTextureFinal, volumeLightLastRT);
        KW_Extensions.SafeDestroy(spotLightTexture, volumeLightMat, ditheringTexture, shadowsForceUpdateAnchor);
    ...
    GameObject GetShadowAnchor() //unity disable shadows rendering if there is no opaque objects in a frustrum. So avoid it just by creating simple small object. 
    {
        if (shadowsForceUpdateAnchor == null)
        {
            shadowsForceUpdateAnchor = Instantiate( Resources.Load<GameObject>("ShadowAnchorFix"));
            shadowsForceUpdateAnchor.hideFlags = HideFlags.HideAndDontSave;
            shadowsForceUpdateAnchor.transform.parent = transform;
            shadowsForceUpdateAnchor.transform.localScale = Vector3.zero;
        }
        return shadowsForceUpdateAnchor;
    }

    public void RenderVolumeLights(Camera currentCamera,  float resolutionScale, float waterHeight, float transparent, int steps, float blurSize,
        Dictionary<CommandBuffer, CameraEvent> waterSharedBuffers)
    {

        if (!isInitialized) Initialize();
        var shadowsAnchor = GetShadowAnchor();
        shadowsAnchor.transform.position = currentCamera.transform.position + currentCamera.transform.forward * 10;
        ...
        SetMaterialParams(currentCamera, waterHeight, transparent, steps, blurSize);
        ...
        if (cb_BlurPass == null || !waterSharedBuffers.ContainsKey(cb_BlurPass))
        {
            ...
            waterSharedBuffers.Add(cb_ClearPass, currentCamera.actualRenderingPath == RenderingPath.Forward ? CameraEvent.BeforeDepthTexture : CameraEvent.BeforeGBuffer);
        }
    ...
    private void UpdateLightBuffer(Camera currentCamera, VolumeLight volumeLight, Dictionary<CommandBuffer, CameraEvent> waterSharedBuffers)
    {
        ...
        if (currentLight.type == LightType.Directional)
        {
            ...
            anisoMie = ComputeMieVector(0.05f);
        }
        ...
        {
            if (!waterSharedBuffers.ContainsKey(cb_light)) waterSharedBuffers.Add(cb_light, currentCamera.actualRenderingPath == RenderingPath.Forward ? CameraEvent.AfterForwardOpaque : CameraEvent.AfterGBuffer);
    ...
    private void SetMaterialParams(Camera cam, float waterHeight, float transparent, int steps, float blurRadius)
    {
        //Shader.SetGlobalTexture("KW_VolumetricLight", volumeLightTextureFinal);
        ...
        var volumeLightMaxDist = Mathf.Max(0.3f, transparent * 2);
       // var volumeLightFade = 1-Mathf.Clamp01((waterHeight - cam.transform.position.y) / volumeLightMaxDist);
        float volumeLightFade = Mathf.Clamp01(Mathf.Exp(-1 * (waterHeight - cam.transform.position.y) / transparent));

        Shader.SetGlobalMatrix("KW_InverseProjectionMatrix", cam.projectionMatrix.inverse);
