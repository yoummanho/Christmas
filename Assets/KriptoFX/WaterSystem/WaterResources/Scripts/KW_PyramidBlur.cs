...
public class KW_PyramidBlur
{
    ...
    public void ComputeBlurPyramid(float blurRadius, RenderTextureTemp sourceID, RenderTextureTemp targetID, CommandBuffer cmd)
    {
       ...
       // cmd.SetGlobalFloat("_SampleScale", 0.5f + logh - logh_i);
        ...
        for (var level = 0; level < iterations; level++)
        {
            tempBuffers1[level] = ReinitializeRenderTextureTemp(tempBuffers1[level], cmd, "_blurPassDown" + level, lastWidth, lastHeight, 0, sourceID.descriptor.colorFormat);
        ...
        {
            var baseSize = blurBufferSizes[level];
            tempBuffers2[level] = ReinitializeRenderTextureTemp(tempBuffers2[level], cmd, "_blurPassUp" + level, baseSize.Width, baseSize.Height, 0, sourceID.descriptor.colorFormat);
