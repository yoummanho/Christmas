...
public static class KW_Extensions {
    ...
    public static bool IsCanExecuteCameraBuffers(Camera cam, WaterSystem water)
    {
    ...
    public static RenderTexture ReinitializeRenderTexture(RenderTexture rt, int width, int height, int depth, GraphicsFormat format,
        Color? clearColor = null, bool useRandomWrite = false, bool useMipMap = false, TextureWrapMode wrapMode = TextureWrapMode.Repeat, FilterMode filterMode = FilterMode.Bilinear)
    {
        if (rt == null || rt.width != width || rt.height != height)
        {
    ...
    public class RenderTextureTemp
    {
        ...
        {
            this.cmd = cmd;
            this.name = name + "_" + width + "_" + height + "_" + depth + "_" + format;
        ...
        public RenderTextureTemp(CommandBuffer cmd, string name, int width, int height, int depth, RenderTextureFormat format, bool isLinear, FilterMode filterMode = FilterMode.Bilinear, bool enableRandomWrite = false, bool useMipMap = false, TextureDimension dimension = TextureDimension.Tex2D, int MSAA = 1, int mipCount = -1)
        {
            this.cmd = cmd;
            this.name = name + "_" + width + "_" + height + "_" + depth + "_" + format;
            id = Shader.PropertyToID(this.name);
            ...
            if (mipCount > 0) descriptor.mipCount = mipCount;
        ...
        public void GetTemporary(CommandBuffer cmd, string name, int width, int height, int depth, RenderTextureFormat format, bool isLinear = true, FilterMode filterMode = FilterMode.Bilinear)
        {
            if (this.cmd == null || this.cmd != cmd || descriptor.width != width || descriptor.height != height)
            {
                if (descriptor.width > 1 && descriptor.height > 1 && this.cmd != null)
                {
                    this.cmd.ReleaseTemporaryRT(id);
                }
                ...
                this.cmd.GetTemporaryRT(id, descriptor, filterMode);
            }
        }

        public void Release()
        {
            if(cmd != null) cmd.ReleaseTemporaryRT(id);
        }
    }

    public static RenderTextureTemp ReinitializeRenderTextureTemp(RenderTextureTemp rt, CommandBuffer cmd, string name, int width, int height, int depth, RenderTextureFormat format, bool isLinear = true, FilterMode filterMode = FilterMode.Bilinear)
    {
        if (rt == null || rt.descriptor.width != width || rt.descriptor.height != height || rt.descriptor.colorFormat != format || rt.cmd != cmd)
        {
            if (rt != null)
            {
                rt.Release();
        ...
        return new RenderTextureTemp(cmd, name, width, height, depth, format, isLinear, filterMode);
    }

    public static RenderTextureTemp ReinitializeRenderTextureTemp(RenderTextureTemp rt, CommandBuffer cmd, string name, int width, int height, int depth, RenderTextureFormat format, bool isLinear, 
        FilterMode filterMode = FilterMode.Bilinear, bool enableRandomWrite = false, bool useMipMap = false, TextureDimension dimension = TextureDimension.Tex2D, int MSAA = 1, int mipCount = -1)
    {
        if (rt == null || rt.descriptor.width != width || rt.descriptor.height != height || rt.descriptor.colorFormat != format || rt.cmd != cmd)
        {
            if (rt != null)
        ...
        return new RenderTextureTemp(cmd, name, width, height, depth, format, isLinear, filterMode, enableRandomWrite, useMipMap, dimension, MSAA, mipCount);
    }
