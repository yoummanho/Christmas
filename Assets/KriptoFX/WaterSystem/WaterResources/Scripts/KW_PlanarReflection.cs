...
public class KW_PlanarReflection : MonoBehaviour
{
    ...
    private RenderTexture reflectionRT_depth;
    //float m_ClipPlaneOffset = 0.1f;

    public void Release()
    {
        KW_Extensions.SafeDestroy(reflCameraGO);
        KW_Extensions.ReleaseRenderTextures(reflectionRT, reflectionRT_depth);
    ...
    void InitializeReflectionTexture(int width, int height)
    {
        reflectionRT = KW_Extensions.ReinitializeRenderTexture(reflectionRT, width, height, 0, RenderTextureFormat.ARGBHalf, null, false, true);
        reflectionRT_depth = KW_Extensions.ReinitializeRenderTexture(reflectionRT_depth, width, height, 24, RenderTextureFormat.Depth, null, false, true);
    }
    ...
    public void RenderPlanar(Camera currentCamera, Vector3 waterPosition, float resolutionScale, List<Material> waterShaderMaterials)
    {
       ...
       // reflectionCamera.allowHDR = true; //bug, without hdr camera doesn't render to texture
        ...
        InitializeReflectionTexture(width, height);
       ...
       // reflectionCamera.backgroundColor = Color.black;
        ...
        reflectionCamera.SetTargetBuffers(reflectionRT.colorBuffer, reflectionRT_depth.depthBuffer);
        ...
        foreach (var mat in waterShaderMaterials)
        {
            ...
            mat.SetTexture("KW_PlanarReflectionDepth", reflectionRT_depth);
        }
    }
