...
public class KW_FFT_HeightData : MonoBehaviour
{
    ...
    {
        HeighDataTexture = KW_Extensions.ReinitializeRenderTexture(HeighDataTexture, size, size, 0, UnityEngine.Experimental.Rendering.GraphicsFormat.R16G16B16A16_SFloat);
        
        if (computeShader == null) computeShader = (ComputeShader)Resources.Load(@"ComputeFFT_Height");
    ...
    public KW_WaterSurfaceData? GetWaterSurfacePositionNormal(Vector3 worldPosition)
    {
       ...
       // Debug.DrawRay(waterPos, rawHeightData[pixelIdx].normal);
        return new KW_WaterSurfaceData() { Position = waterPos, Normal = surfaceData .normal};
    }
    ...
    public void UpdateHeightData(int size, float domainScale, float _waterHeight)
    {
        ...
        Graphics.SetRenderTarget(HeighDataTexture);
...
public struct KW_WaterSurfaceData
{
