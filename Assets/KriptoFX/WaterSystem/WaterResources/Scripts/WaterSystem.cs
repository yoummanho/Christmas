...
public class WaterSystem : MonoBehaviour
{
   ...
   // public float SSR_ReflectedImageEffectsHDR_Mul;
    ...
    public float UnderwaterBlurRadius = 2.3f;
...
#region private variables
   ...
   // [SerializeField]
    ...
    private KW_CameraReflection_HDRP _cameraReflection;
    private KW_CameraReflection_HDRP cameraReflection
    {
        get
        {
            if (_cameraReflection == null) _cameraReflection = tempGameObject.AddComponent<KW_CameraReflection_HDRP>();
    ...
    private KW_WaterSystemVolumes_HDRP _waterVolumesHDRP;
    private KW_WaterSystemVolumes_HDRP waterVolumesHDRP
    {
        get
        {
            if (_waterVolumesHDRP == null) _waterVolumesHDRP = tempGameObject.AddComponent<KW_WaterSystemVolumes_HDRP>();
            return _waterVolumesHDRP;
        }
    }
...
#region ShaderKeywords
    ...
    private void RenderPipelineManager_beginCameraRendering(ScriptableRenderContext context, Camera cam)
    {
        ...
        UpdateManual();
        if (!Application.isPlaying && currentCamera == null && Camera.main == null) Debug.LogError("You should tag your main camera with the tag 'MainCamera'");
    }
    ...
    async void UpdateManual()
    {
        ...
        waterVolumesHDRP.UpdateWaterPasses();

        UpdateShaderParameters();
    }

    void UpdateReflectionManual()
    {
        if (!isWaterInitialized) return;
        if (ReflectionMode == ReflectionModeEnum.PlanarReflection) RenderPlanarReflection();
    ...
    {
#if UNITY_EDITOR
        if (sceneCamera == null) FindSceneCamera();
        var focusedWindow = EditorWindow.focusedWindow;
        if (focusedWindow != null)
        {
            if (focusedWindow.titleContent.text.Contains("Game")) currentCamera = Camera.main;
            else if (focusedWindow.titleContent.text.Contains("Scene")) currentCamera = sceneCamera;
        }
        else currentCamera = sceneCamera;
#else
    ...
    {
        if (SceneView.lastActiveSceneView != null) sceneCamera = SceneView.lastActiveSceneView.camera;
        else
        {
            var camCurrent = Camera.current;
            if (camCurrent != null && camCurrent.cameraType == CameraType.SceneView) sceneCamera = camCurrent;
        }
    }
#endif
    ...
    void UpdateFFT_HeightData()
    {
       ...
       // fft_HeightData.UpdateHeightData(512, DomainSize, transform.position.y);
    }
    ...
    {
        cameraReflection.RenderPlanar(currentCamera, transform.position, ReflectionTextureScale*0.75f, FixUnderwaterSkyReflection, waterSharedMaterials);
    }

    void RenderSkyReflection()
    {
        cameraReflection.RenderCubemap(currentCamera, transform.position, CubemapUpdateInterval, CubemapCullingMask, CubemapTextureSize, FixUnderwaterSkyReflection, waterSharedMaterials);
    }
    ...
    void RenderCausticEffect()
    {
        ...
        causticRendering.ComputeCausticTextures(currentCamera, CausticStrength, CausticDepthScale, CausticTextureSize, CausticActiveLods, CausticMeshResolution,
            UseCausticBicubicInterpolation, UseCausticDispersion, UseDepthCausticScale, dispersionStrength, waterSharedMaterials,  waterGUID);
...
#region  Shoreline Methods
    ...
    {
        SphericalHarmonicsL2 harmonics;
        ...
        {
            if(ambient != Color.black) lastAmbient = ambient;
            return lastAmbient;
        }
    }
    ...
    public async void UpdateShaderParameters()
    {
        ...
        var hdCamera = UnityEngine.Rendering.HighDefinition.HDCamera.GetOrCreate(currentCamera);
        
       // currentCamera.historyRTHandleProperties.rtHandleScale
        Shader.SetGlobalVector("_CurrentRTHandleSize", hdCamera.screenSize);
        Shader.SetGlobalVector("_CurrentRTHandleScale", hdCamera.historyRTHandleProperties.rtHandleScale);
    ...
    public KW_WaterSurfaceData? GetWaterSurfacePositionNormal(Vector3 worldPosition)
    {
        if (!isWaterInitialized || !isBuoyancyDataReadCompleted) return new KW_WaterSurfaceData { Position = worldPosition, Normal = Vector3.up };
        return fft_HeightData.GetWaterSurfacePositionNormal(worldPosition);
    }
