...
//	uv = clamp(uv * _CurrentRTHandleSize.xy * _CameraDepthTexture_TexelSize.xy, 0.001, 0.999);
//	float z = UNITY_SAMPLE_TEX2DARRAY(_CameraDepthTexture, float3(uv, 0));
//	return z;
//#endif
//	return 1;
//}
...
//{
//#if defined(WATER_HDRP)
//	uv = clamp(uv * _CurrentRTHandleScale.xy, 0.001, 0.999);
//	float3 color = UNITY_SAMPLE_TEX2DARRAY_LOD(_ColorPyramidTexture, float3(uv, 0), 0);
//	return color;
//#endif
//	return 1;
//}
...
//	return  _ExposureTexture.SampleLevel(sampler_linear_clamp, float2(0, 0), 0);
//#else
//	return 1;
//#endif
//}

//float3 GetAmbientColorExposure(float3 rawAmbientColor)
//{
//	return rawAmbientColor * GetCurrentExposureMultiplier();
//}

inline half3 NormalLocalToEye(half3 normal)
{
	float4x4 MATRIX_IT_MV = transpose(mul(UNITY_MATRIX_I_M, UNITY_MATRIX_I_V));
	normal.xz = (mul((float3x3)MATRIX_IT_MV, half3(normal.x, 0, normal.z))).xz;
	normal.xz = normal.xz;
	return normal;
}
...
inline half3 ComputeDetailedNormal(half3 normal, sampler2D detailedTex, float2 domainSize_Detail, float3 worldPos, half detailMaxDistance)
{
	...
	half3 normal_detail = tex2D(detailedTex, worldPos.xz / domainSize_Detail).xyz;
...
{
	half4 color = tex.Sample(state, uv.xy);
	half lum = dot(color.xyz, float3(0, 1, 0));
...
half3 ComputeSunlight(half3 normal, half3 viewDir, half linearFresnel, float3 lightDir, float3 lightColor, half shadowMask, float viewDistNormalized, half KW_Transparent)
{
