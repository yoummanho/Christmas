...
half4 ComputeWaterColor(v2f i, float facing)
{
	...
	i.viewDir = GetWorldSpaceNormalizeViewDir(GetCameraRelativePositionWS(i.worldPos));
...
//#endif
	...
	float2 wavesMapUV = (i.worldPos.xz - KW_ShorelineAreaPos.xz) / KW_WavesMapSize + 0.5;
	float4 uv_angle_alpha = tex2Dlod(KW_BakedWaves1_UV_Angle_Alpha, float4(wavesMapUV, 0, 0));
	float2 timeOffset_Scale = tex2Dlod(KW_BakedWaves1_TimeOffset_Scale, float4(wavesMapUV, 0, 0));
	//return float4(uv_angle_alpha.w, 0, 0, 1);
#endif
...
#else
	reflection = SAMPLE_TEXTURECUBE(KW_ReflectionCube, sampler_linear_clamp, reflVec);
	reflection *= exposure;

	#if SSPR_REFLECTION
		float2 refl_uv = ComputeScreenSpaceReflectionDir(normalReflection, i.viewDir);
		refl_uv.y -= KW_SSR_ClipOffset;
		float4 ssr = SampleBicubic(KW_ScreenSpaceReflectionTex, sampler_linear_clamp, refl_uv, KW_ScreenSpaceReflectionTex_TexelSize).xyzw;
#if defined(USE_FOG)
		ssr.rgb = ssr.rgb - atmosColor + ssr.rgb * atmosOpacity * 0.25;
#endif
	...
	float z = SampleCameraDepth(i.screenPos.xy / i.screenPos.w);
	half surfaceTensionFade = saturate((LinearEyeDepth(z, _ZBufferParams) - i.screenPos.w) * 8);
	...
	float sceneZRefr = LinearEyeDepth(tex2D(_CameraDepthTextureBeforeWaterZWrite_Blured, i.screenPos.xy / i.screenPos.w), _ZBufferParams);
	float surfaceDepthRefr = (i.screenPos.w);
	float fadeRefr = clamp((sceneZRefr - surfaceDepthRefr) * depthFix, 0.5, min(KW_Transparent, 5));
	

	float3 refractedRay = ComputeWaterRefractRay(-i.viewDir, normal, fadeRefr * 2 * surfaceTensionFade);
	...
	half3 refraction = SampleCameraColor((refractionScreenPos.xy / refractionScreenPos.w));

	float sceneZ = LinearEyeDepth(SampleCameraDepth(refractionScreenPos.xy / refractionScreenPos.w));
	...
	if (fade < 0)
	{
		...
		sceneZ = LinearEyeDepth(SampleCameraDepth(i.screenPos.xy / i.screenPos.w), _ZBufferParams);
		fade = (sceneZ - surfaceDepth);

		refraction = SampleCameraColor((i.grabPos.xy / i.grabPos.w));
		volumeScattering = tex2D(KW_VolumetricLight, i.screenPos.xy / i.screenPos.w);
	}
...
#if defined(USE_FOG)
	refraction.rgb = refraction.rgb - atmosColor + refraction.rgb * atmosOpacity * 0.25;
#endif

	underwaterColor = ComputeUnderwaterColor(refraction, volumeScattering.rgb, fade, KW_Transparent, KW_WaterColor, KW_Turbidity, KW_TurbidityColor);
	...
	underwaterColor = lerp(underwaterColor, clamp(GetAmbientColor() * exposure * 0.125 + volumeScattering.a * 0.5 +  volumeScattering.rgb * 0.25, 0, 0.75), foamTex * foamMask);
	...
	if (_DirectionalShadowIndex >= 0)
	{
		...
		dirLight.color *= exposure;
		finalColor += ComputeSunlight(normal, i.viewDir, linearFresnel, -dirLight.forward, dirLight.color, volumeScattering.a, viewDistNormalized, KW_Transparent);
	}
...
#if defined(USE_FOG)
	finalColor.rgb = finalColor.rgb * (1 - atmosOpacity) + atmosColor;
#endif
	return float4(max(0, finalColor), surfaceTensionFade);
}

v2fDepth vertDepth(float4 vertex : POSITION, float underwaterCubeMask : COLOR0)
{
	v2fDepth o;
	o.worldPos = GetAbsolutePositionWS(mul(UNITY_MATRIX_M, vertex));
...
v2f ComputeVertexInterpolators(v2f o, float3 worldPos, float4 vertex : POSITION)
{
	...
	o.worldPosRefracted = mul(UNITY_MATRIX_M, vertex).xyz;
