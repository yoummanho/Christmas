	...
	Subshader
	{
		...
		Pass
		{
			...
			v2f vert(appdata_t v)
			{
				...
				o.texcoord = mul(UNITY_MATRIX_M, float4(v.vertex.xyz, 1)).xz / KW_FFTDomainSize;

				float3 viewRay = UnityObjectToViewPos(v.vertex).xyz;
				o.ray.w = viewRay.z;
				viewRay *= -1;
				o.ray.xyz = mul((float3x3)mul(UNITY_MATRIX_I_M, UNITY_MATRIX_I_V), viewRay);
				o.rayCameraOffset = mul(mul(UNITY_MATRIX_I_M, UNITY_MATRIX_I_V), float4(0, 0, 0, 1)).xyz;
			...
			half4 frag(v2f i) : SV_Target
			{
				...
				float depth = SampleCameraDepth(i.screenUV.xy / i.screenUV.w);
